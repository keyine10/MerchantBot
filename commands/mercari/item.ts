...
import { MercariItem, MercariItemInfo, MercariItemTranslation, MercariURLs, MercariItemConditionId } from '../../mercari/types';
import logger from '../../utils/logger';

// Export a function to get item details and translation and return the embed objects
export async function getItemDetailViewModel(itemId: string): Promise<{ embeds: APIEmbed[]; content?: string; photoUrls?: string[] }> {
	...
	try {
		...
		const thumbnailUrl = typeof item.photos[0] === 'string' ? item.photos[0] : item.photos[0]?.uri;
	...
	async execute(interaction: ChatInputCommandInteraction) {
		...
		if (result.photoUrls && result.photoUrls.length > 0) {
			try {
				...
				const attachments = downloadResults.filter((attachment): attachment is AttachmentBuilder => 
					attachment !== null
				);
				
				if (attachments.length > 0) {
					await interaction.followUp({
						content: `📸 **Item Photos** (${attachments.length} ${attachments.length === 1 ? 'photo' : 'photos'}):`,
						files: attachments,
					});
				}
			} catch (error) {
