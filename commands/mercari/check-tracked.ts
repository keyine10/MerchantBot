        ...
        try {
            ...
            if (trackedQueries.length === 0) {
                return interaction.editReply({
                    content: 'You don\'t have any tracked queries. Use `/list-queries` to enable tracking for your queries.',
                });
            }
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(0x0099ff)
                .setDescription(`Checking ${trackedQueries.length} tracked queries for new items...`)
                .addFields(
                    ...trackedQueries.map(query => ({
                        name: query.name,
                        value: `**Keyword:** ${query.searchParams.keyword || 'None'}\n**Tracked:** ✅`,
                        inline: true
                    }))
                )
                .setTimestamp()
