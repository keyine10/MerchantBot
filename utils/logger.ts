...
class Logger {
      ...
      for (let i = 4; i < lines.length; i++) {
        ...
        if (
          ...
          if (match) {
            ...
            const func =
              functionName && functionName !== "Object.<anonymous>"
                ? functionName
                : "anonymous";
  ...
  private writeToFile(filename: string, message: string) {
    try {
      ...
      const formattedMessage =
        this.formatMessage("INFO", message, caller) + "\n";
  ...
  error(
    ...
    try {
      ...
      const formattedMessage =
        this.formatMessage("ERROR", finalMessage, caller) + "\n";
  ...
  warn(message: string, filename: string = "app.log") {
    try {
      ...
      const formattedMessage =
        this.formatMessage("WARN", message, caller) + "\n";
      const logFile = path.join(this.logDir, filename);
