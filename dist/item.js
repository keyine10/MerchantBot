...
const mercari_js_1 = __importDefault(require("../../mercari/mercari.js"));
const utils_js_1 = require("../../mercari/utils.js");
...
async function getItemEmbeds(itemId, interaction) {
    try {
        const data = await mercari_js_1.default.getItemDetails(itemId);
        if (!data || !data.data) {
            return {
                embeds: [{
        ...
        const translationData = await mercari_js_1.default.getItemTranslation(itemId);
        const item = data.data;
        const itemOverviewEmbed = {
            title: translationData.name.substring(0, 100),
            url: utils_js_1.MercariURLs.ROOT_PRODUCT + item.id,
            author: {
                ...
                url: `${utils_js_1.MercariURLs.USER_PROFILE}${item.seller.id}`,
            },
